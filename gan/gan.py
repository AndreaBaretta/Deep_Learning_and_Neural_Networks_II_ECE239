...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    mseloss = nn.MSELoss()
    loss = 0.5*mseloss(scores_real, torch.ones_like(scores_real).type(dtype))
    loss += 0.5*mseloss(scores_fake, torch.zeros_like(scores_fake).type(dtype))
...
def ls_generator_loss(scores_fake):
    ...
    mseloss = nn.MSELoss()
    loss = 0.5*mseloss(scores_fake, torch.ones_like(scores_fake).type(dtype))
