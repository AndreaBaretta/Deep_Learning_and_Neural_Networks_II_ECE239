...
class CVAE(nn.Module):
    ...
    def forward(self, x, c):
        ...
        ############################################################################################
        # Replace "pass" statement with your code
        x_ = self.encoder(torch.cat((torch.flatten(x, start_dim=1), c), dim=1))
        mu = self.mu_layer(x_)
        logvar = self.logvar_layer(x_)
        z = reparametrize(mu, logvar)
...
def loss_function(x_hat, x, mu, logvar):
    ...
    ################################################################################################
    # Replace "pass" statement with your code
    bce_loss = nn.functional.binary_cross_entropy(x_hat, x, reduction='none').sum(axis=(1,2,3)).mean()
    kl_loss = -0.5*(1 + logvar - mu**2 - torch.exp(logvar)).sum(axis=1).mean()
