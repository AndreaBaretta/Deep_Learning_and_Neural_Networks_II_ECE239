...
class DQN:
    ...
    def _optimize_model(self):
        ...
        with torch.no_grad():
            y = rewards + (1 - dones) * self.gamma*self.model(next_states).max(dim=1)[0]
...
#                 if self.wandb:
#                     wandb.log({'total_reward': total_reward, 'loss': None})
#                 print(f"Episode: {episode}: Time: {time.time() - start_time} Total Reward: {total_reward} Avg_Loss: None (no optimization steps)")
...
#         return training_rewards, training_losses, validation_rewards
