...
class EnvWrapper(gym.Wrapper):
    ...
    def reset(self, **kwargs):
        ...
        for _ in range(self.initial_no_op):
            obs, _, done, _, _ = self.env.step(self.do_nothing_action)
            if done:
                obs, _ = self.env.reset(**kwargs)
        obs = preprocess(obs)
        self.frames = [obs] * self.stack_frames
    ...
    def step(self, action):
        ...
        self.frames.pop(0)
        return np.stack(self.frames, axis=0), total_reward, done, truncated, info